...
const ProfileEditModal = ({ profile, setProfile,newUser }) => {
    ...
    fetch(url, {
      ...
      .then((result) => {
        if (result.success) {
          ...
          const url = `http://localhost:5000/newUser/${email}`;
          console.log(newUser);
          fetch(url, {
            method: "PUT",
            headers: {
            ...
            .then((inserted) => {
              if (inserted.insertedId) {
                ...
                reset();
              } else toast.error(`user ${data.name} Updated to failed`);
            });
        }
